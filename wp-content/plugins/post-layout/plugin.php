...
function pstl_comment_text($comment = '') 
{
    ...
    if (get_comment_ID() == $_GET['cid']) return $comment . $options['comment_after'];
...
function pstl_the_content($content)
{
    ...
    if ($options['multiauthor'])
    {
        ...
        if (is_array($user_options)) $options = array_merge($options, $user_options);
    }
    ...
    if (is_single())
    {
        ...
        if (!$pstl_before_disabled)
        {
            ...
            if ($before == '') $before = $options['post_before' . $suffix];
            else $before = str_replace('[default]', $options['post_before' . $suffix], $before);
        }

        if (!$pstl_after_disabled)
        {
            $after = trim($options['post_after' . $suffix . $list[0]->cat_ID]);
            if ($after == '') $after = $options['post_after' . $suffix];
            else $after = str_replace('[default]', $options['post_after' . $suffix], $after);
        }
        ...
        if ($options['post_more_size'] > 0)
        {
            ...
            if ($length >= $options['post_more_size'])
            {
                ...
                if ($more == '') $more = $options['post_more' . $suffix];
                else $more = str_replace('[default]', $options['post_more' . $suffix], $more);
            }
        }
        else
        {
...
function pstl_insert_blocks($code)
{
    ...
    for ($x=1; $x<5; $x++)
    {
        if ($options['block_name_' . $x] == '') continue;
        $date = $options['block_end_' . $x];
        if ($date != '')
        {
            $date = explode(',', $date);
            $time = @mktime(0, 0, 0, $date[1], $date[2], $date[0]);
            if ($time === -1 || $time === false) continue;
            if (time() > $time)
...
if (isset($_POST['pstl_edit']))
{
    ...
    if (isset($_POST['pstl_after_disabled'])) add_post_meta($post_id, 'pstl_after_disabled', '1', true);
    else delete_post_meta($post_id, 'pstl_after_disabled');

    if (isset($_POST['pstl_more_disabled'])) add_post_meta($post_id, 'pstl_more_disabled', '1', true);
    else delete_post_meta($post_id, 'pstl_more_disabled');

    if (isset($_POST['pstl_before_disabled'])) add_post_meta($post_id, 'pstl_before_disabled', '1', true);
    else delete_post_meta($post_id, 'pstl_before_disabled');
}
}
...
$agents = explode(',', "elaine/3.0,iphone,ipod,palm,eudoraweb,blazer,avantgo,windows ce,cellphone,small,mmef20,danger,hiptop,proxinet,newt,palmos,netfront,sharp-tq-gx10,sonyericsson,symbianos,up.browser,up.link,ts21i-10,mot-v,portalmmm,docomo,opera mini,palm,handspring,nokia,kyocera,samsung,motorola,mot,smartphone,blackberry,wap,playstation portable,lg,mmp,opwv,symbian,epoc");
foreach ($agents as $a)
{
